...
public class ReportDataDogService {
    ...
    @Autowired
    public ReportDataDogService(AzolikFizRepository azolikFizRepository, AzolikYurRepository yurReportRepository, DokRepository dokRepository, KreditRepository kreditRepository, SaldoRepository saldoRepository, ZalogRepository zalogRepository, GrafikRepository grafikRepository) {
    ...
    public File getReport(LocalDate startDate, LocalDate endDate) {
        List<Kredit> kreditList = kreditRepository.findActiveInPeriod(startDate, endDate);
        ...
        try {
            ...
            List<CrifKreditDTO> kreditDTOList = kredits.stream().map(kredit -> {
                ...
                System.out.println("Заполняем refDates:");
                ...
                while (current.isBefore(endDate) || current.equals(endDate)) {
                    ...
                    if (endOfMonth.isBefore(endDate) || endOfMonth.equals(endDate)) {
                        ...
                        System.out.println("Добавлено в refDates: " + endOfMonth + " (END_OF_MONTH)");
                        current = endOfMonth.plusDays(1);
                    } else {
                        if (!endDate.equals(endOfMonth)) {
                            refDates.put(endDate, "END_DATE");
                            System.out.println("Добавлено в refDates: " + endDate + " (END_DATE)");
                        }
                        break;
                    }
                }
...
//                        .map(GrafikDTO::getDats)
//                        .count());
//                System.out.println("days " + outstandingPaymentNumber);

                boolean isFirstIteration = true; // Флаг для первой итерации
                int gd = 0;

                // Отфильтровать refDates, оставив только те, которые начинаются с даты договора или совпадают по месяцу и году с датами из графика кредита
                List<Map.Entry<LocalDate, String>> filteredRefDates = refDates.entrySet().stream()
                        .filter(entry ->
                                !entry.getKey().isBefore(kreditDTO.getDatadog()) || // Убедиться, что refDate не раньше даты договора
                                        kreditDTO.getGrafiks().stream().anyMatch(grafik ->
                                                grafik.getDats().getMonth() == entry.getKey().getMonth() &&
                                                        grafik.getDats().getYear() == entry.getKey().getYear())
                        ).collect(Collectors.toList());

                for (Map.Entry<LocalDate, String> entry : filteredRefDates) {

//                    int grafikDats = grafikRepository.countGrafikByNumdog(kreditDTO.getNumdog());

                    LocalDate refDate = entry.getKey();


                    Integer grafikDats = grafikRepository.findGrafikByNumdogAndDats(kreditDTO.getNumdog(), refDate);



                    System.out.println(refDate);

                    LocalDate datadog = kreditDTO.getDatadog();
                    LocalDate datsZakr = kreditDTO.getDatsZakr();

//                    if (kreditDTO.getGrafiks().stream().anyMatch(grafik ->
//                            grafik.getDats().getMonth() == refDate.getMonth() &&
//                                    grafik.getDats().getYear() == refDate.getYear()) || datsZakr == null || !refDate.isBefore(datadog) ) {
                    ...
                    BigDecimal saldoSumsLsKred = getSaldoSumsByLsAndClosestDats(kreditDTO.getLsKred(), refDate);
                    if (saldoSumsLsKred == null) {
                        saldoSumsLsKred = BigDecimal.ZERO;
                    }


                    BigDecimal saldoSumsLsProc = getSaldoSumsByLsAndClosestDats(kreditDTO.getLsProc(), refDate);
                    if (saldoSumsLsProc == null) {
                        saldoSumsLsProc = BigDecimal.ZERO;
                    }
//

                    BigDecimal saldoSumsLsprosrKred = getSaldoSumsByLsAndClosestDats(kreditDTO.getLsprosrKred(), refDate);
                    if (saldoSumsLsprosrKred == null) {
                        saldoSumsLsprosrKred = BigDecimal.ZERO;
                    }
//
                    BigDecimal saldoSumsLsPeni = getSaldoSumsByLsAndClosestDats(kreditDTO.getLspeni(), refDate);
                    if (saldoSumsLsPeni == null) {
                        saldoSumsLsPeni = BigDecimal.ZERO;
                    }
//
                    ...
                    int outstandingBalance = (saldoSumsLsKred.intValue() + saldoSumsLsProc.intValue()
                            ...
                            + (saldoSumsLsSudKred != null ? saldoSumsLsSudKred.intValue() : 0));

                    if (status.equals("CA") || status.equals("CL")) outstandingBalance = 0;

                    if (outstandingBalance < 0) outstandingBalance = 0;


                    BigDecimal pogKredForMax = kreditDTO.getGrafiks().stream()
                            .max(Comparator.comparing(GrafikDTO::getPogKred)) // поиск максимальной даты
                    ...
                    if (status.equals("CA") || status.equals("CL")) sumsForMaxDate = BigDecimal.valueOf(0);

                    if (sumsForMaxDate.intValue() < 0) sumsForMaxDate = BigDecimal.valueOf(0);

                    BigDecimal pogProcForMax = kreditDTO.getGrafiks().stream()
                            .max(Comparator.comparing(GrafikDTO::getPogProc)) // поиск максимальной даты
                    ...
                    if (status.equals("CA") || status.equals("CL")) {
                        ...
                        interestOverduePaymentsNumber = sumsForMaxDate.doubleValue() / pogProcForMax.doubleValue();
                    }


                    if (pogKredForMax.intValue() == 0) {
                        principalOverduePaymentNumber = 0;
                    } else {
                        principalOverduePaymentNumber = principalOverduePaymentAmount / pogKredForMax.doubleValue();
                    }
...
//                    int outstandingPaymentNumber = grafikDats - gd;
//                    if (grafikDats == null)

                    int outstandingPaymentNumber = grafikDats != null ? grafikDats : 0;

                    System.out.println("day " + outstandingPaymentNumber + "\n refDate " + refDate);

                    isFirstIteration = false;

                    if (outstandingBalance == 0) outstandingPaymentNumber = 0;
                    ...
                    if (status.equals("AC") && outstandingPaymentNumber < 0) outstandingPaymentNumber = 1;
...
// Сохраняем текущее значение overduePeriod для последующего использования
                    ...
                    if (kreditDTO.getDatadog().getYear() == refDate.getYear() &&
                            kreditDTO.getDatadog().getMonthValue() == refDate.getMonthValue()) {
                        overduePeriod = 0;
                    }
...
// Проверяем, совпадают ли текущие значения с предыдущими и отличаются ли от "0"
                    if (currentNumdog.equals(prevNumdog) && status.equals("AC") && currentKod.equals(prevKod) &&
                            (currentPod == prevPod || (currentPrincipalOverduePaymentAmount == prevPrincipalOverduePaymentAmount &&
                                    sumsForMaxDate.intValue() == prevSumsForMaxDate && prevPrincipalOverduePaymentAmount != 0 && prevSumsForMaxDate != 0))) {
...
// Сохраняем текущие значения для следующей итерации
                    ...
                    System.out.println(kreditDTO.getNumdog() + " " + refDates.size());
                                    ...
                                    if (!kreditDTO.getDatadog().isAfter(refDate) && !(firstPaymentDate == null) && !(latestDate == null) && (countedGrafik >= 1)) {
                                        if (isUnique) {
                                            dataBuilder.append("CI|MKOR0001||")
                                                    ...
                                                    .append("|")
                                                    .append((int) Math.ceil(interestOverduePaymentsNumber))
                                                    .append("|")
...
//                                        }
    ...
    public BigDecimal getSaldoSumsByLsAndClosestDats(String ls, LocalDate date) {
        LocalDate closestDate = saldoRepository.findClosestDateByLs(ls, date);
        if (closestDate != null) {
            return saldoRepository.findSumsByLsAndDats(ls, closestDate);
        } else {
