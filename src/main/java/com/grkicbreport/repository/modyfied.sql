...
WHERE k.dats_izm BETWEEN '2023-11-01'
    AND '2023-11-30'
  AND EXISTS (SELECT 1
              FROM saldo s
              WHERE (s.ls = k.lspeni
...
SELECT distinct af.kodchlen,
                ...
                k.numdog,
                k.vidkred,
                max(k.dats_izm_grafik)                                            AS dats_izm,
                MAX(k.sost)                                                  AS sost,
                MAX(k.status)                                                AS status,
                MAX(k.datadog)                                               AS datadog,
                k.dats_zakr                                                  AS dats_zakr,
...
FROM kredit k
     ...
     (select data_priem, data_vozvrat, kod_dog
      from zalog_xranenie) zx on k.kod_dog = zx.kod_dog
WHERE k.dats_izm_grafik BETWEEN '2023-11-01' AND '2023-11-30'
  AND EXISTS (SELECT 1
              FROM saldo s
              WHERE (s.ls = k.lspeni
                  OR s.ls = k.lsproc
